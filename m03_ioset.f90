   ...
   loop_line : do while (ierr .eq. 0)
    ...
    line_scan : do while( pos_start .le. len_eff)
      ...
      select case (cur_key)
        ...
        case (1)
          cur_var='numips (number of incident lights)'
          rval=0d0
...
!*********************************************************
!  processing input section 1
!*********************************************************

subroutine ProcInp_sec1
   use mod_optic 
   ...
   data list_keyword /'npsdbg','nutdbg'/
   integer :: cout_keyword(num_keyword)=0
   ...
   loop_line : do while (ierr .eq. 0)
    ...
    line_scan : do while( pos_start .le. len_eff)
      ...
      select case (cur_key)
        ...
        case (1)
          cur_var='npsdbg (debug output for first npsdbg reflections/refractions)'
          rval=0d0
          read(lineseg,*,err=1002,end=1002) rval(1)
          !error check
        ...
        case (2)
          cur_var='nutdbg (debug output for first npsdbg reflections/refractions)'
          rval=0d0
          read(lineseg,*,err=1002,end=1002) rval(1)
          !error check
          if(rval(1) .ge. -1.0) then
            ...
            !to be refine error message
            write(disp_msg(1),"('invalid nutdbg value ')") 
            call ShowMsg(1)
          endif
